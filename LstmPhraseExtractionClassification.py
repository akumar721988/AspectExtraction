    ...
    if len(lst) > 0 :
        ...
        for index, t in enumerate(lst):
            if t[2] == "B-KEYWORD" and len(prev) == 0:
                temp.append((t[0]+"-"+str(index+1),t[0],t[1]))
                prev.add("B-KEYWORD")
...
def find_i_keyword_set(lst):
    ...
    for index, record in enumerate(lst):
        if record[2] == "I-KEYWORD" and len(prev) == 0:
            temp.append((record[0]+"-"+str(index+1),record[0],record[1]))
            prev.add("I-KEYWORD")
...
class LSTMPhraseExtraction:

    __instance = None
    feature_opinion_model = None

    @staticmethod
    def getInstance():
        """Static Access Method"""
        if LSTMPhraseExtraction.__instance == None:
            LSTMPhraseExtraction()
        return LSTMPhraseExtraction.__instance

    def __init__(self):

        """virtual private constructor"""
        if LSTMPhraseExtraction.__instance == None:
            self.feature_opinion_model = load_model(model_path+"/model.h5")
            f1 = open(model_path+'/idx2Label.pickle',"rb")
            f2 = open(model_path+'/word2Idx.pickle',"rb")
            f3 = open(model_path+'/label2Idx.pickle',"rb")
            self.idx2Label = pickle.load(f1)
            self.word2Idx = pickle.load(f2)
            self.label2Idx = pickle.load(f3)
        LSTMPhraseExtraction.__instance = self

    def getPhrases(self, comment):
        ...
        train_set = padding(createMatrices(trainSentences, self.word2Idx,  self.label2Idx, pos2Idx, char2Idx, context2Idx))
        results = tag_dataset(self.feature_opinion_model,train_set, self.idx2Label,trainSentences)

        for sent in results:
            comment = " ".join([t[0] for t in sent])
            keyword_phrase_results = self.find_keyword_set(sent)
            #keyword_phrase_results_extra = self.find_i_keyword_set(sent)
            #keyword_phrase_results = keyword_phrase_results + keyword_phrase_results_extra
            if len(keyword_phrase_results) > 0 :
                temp = {"comment":comment,"phrases":keyword_phrase_results}
                complete_results.append(temp)

            #else:
            #    keyword_phrase_results = self.find_i_keyword_set(sent)
            #    if len(keyword_phrase_results) > 0 :
            #        temp = {"comment":comment,"phrases":keyword_phrase_results}
            #        complete_results.append(temp)               
    ...
    def find_i_keyword_set(self,lst):
        keyword_found = []
        if len(lst) > 0 :
            temp = []
            prev = set()
            for index, t in enumerate(lst):
                if t[2] == "I-KEYWORD" and len(prev) == 0:
                    temp.append((t[0]+"-"+str(index+1),t[0],t[1]))
                    prev.add("I-KEYWORD")
                elif t[2] == "I-KEYWORD" and "I-KEYWORD" in prev and len(temp) > 0 :
                    temp.append((t[0]+"-"+str(index+1),t[0],t[1]))
                    prev.add("I-KEYWORD")
                elif t[2] == "O" and len(temp) > 1 and "I-KEYWORD" in prev:
                    phrase =  " ".join([k[0] for k in temp])
                    keyword_found.append({
                        "phrase":phrase,
                        "tag":temp
            ...
            if len(temp) > 1 and "I-KEYWORD" in prev :
                    phrase =  " ".join([k[0] for k in temp])
                    keyword_found.append({
                        "phrase":phrase,
                        "tag":temp
...
def test():
    ...
    #df = pd.DataFrame([{
    #    "SentenceText":"The appartment can be a bit noisey (plumbing, neighbors banging around, garbage dissposal etc)  The aprtments seemed to have been designed to rely on carpeting for noise dampening, and now many have laminate flooring, so they have becom thunderous."
    #}])

    obj = LSTMPhraseExtraction.getInstance()
    ...
    for index, row in df.iterrows():
        ...
        results = obj.getPhrases(content)
        if len(results) > 0 :
            complete_results.extend(results)
        else:
            phrase_not_found_comments.append({"comment":content})
